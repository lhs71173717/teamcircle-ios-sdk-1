...
extension AlamofireDecodableError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case let .errorMessage(message):      return message
        }
    }
}
...
extension DataRequest {
    ...
    private static func decodeToObject<T: Decodable>(byKeyPath keyPath: String, decoder: JSONDecoder, response: HTTPURLResponse?, data: Data?) -> Result<T> {
        ...
        case .success(let json):
            if JSONSerialization.isValidJSONObject(json), let success = (json as AnyObject).value(forKeyPath: "resultStatus") as? Bool {
                if success {
                ...
                } else if let errorMessage = (json as AnyObject).value(forKeyPath: "errorMessage") as? String {
                    return .failure(AlamofireDecodableError.errorMessage(message: errorMessage))
                } else {
                    return .failure(AlamofireDecodableError.invalidKeyPath)
                }
            } else {
    ...
    @discardableResult
    func responseDecodableObject<T: Decodable>(queue: DispatchQueue? = nil, keyPath: String? = "resultData", decoder: JSONDecoder = JSONDecoder(), completionHandler: @escaping (DataResponse<T>) -> Void) -> Self {
